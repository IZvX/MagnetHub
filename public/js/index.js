...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function fetchAndDisplayMedia(lang_id = null, name = 'null', type = 'movie', category = 'popular', timeWindow = null, container = contentDiv) {
    try {
      ...
      if (data.results) {
        displayMediaCards(data.results, container, name, type, lang_id); // Pass type and category as list name
      } else {
        container.innerHTML = `<p>No results found for ${listName}.</p><br>${apiUrl}<br>type:${type}<br>cat:${category}`;
      }
    } catch (error) {
  ...
  function displayMediaCards(media, container, listName, type, lang_id) {
    // Clear existing content
    // container.innerHTML = '';

    // Create the MovieScrollList
    const movieList = MovieScrollList(listName, lang_id);
  ...
  function loadHomePageContent() {
    ...
    switch (currentPage) {
      case 'index.html':
        fetchAndDisplayMedia("explorePopularMovie", 'Popular - Movie', 'movie', 'popular');
        fetchAndDisplayMedia("exploreTopRatedMovie", 'Top Rated - Movie', 'movie', 'top_rated');
        fetchAndDisplayMedia("exploreTrendingMovie", 'Trending - Movie', 'movie', 'trending', 'day');  // Trending movies for the day
        fetchAndDisplayMedia("explorePopularTv", 'Popular - TV', 'tv', 'popular');
        fetchAndDisplayMedia("exploreTopRatedTv", 'Top Rated - TV', 'tv', 'top_rated');
        fetchAndDisplayMedia("exploreTrendingTv", 'Trending - TV', 'tv', 'trending', 'day');   // Trending TV shows for the day
        break;
      case "tv.html":
        fetchAndDisplayMedia("explorePopularTv", 'Popular - TV', 'tv', 'popular');
        fetchAndDisplayMedia("exploreTopRatedTv", 'Top Rated - TV', 'tv', 'top_rated');
